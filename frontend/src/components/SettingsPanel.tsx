...
type SettingsTab = 'connection' | 'prompts' | 'agents' | 'history'
...
export function SettingsPanel({
  ...
  const contentMax =
    tab === 'connection' ? 'max-w-xl' : 'max-w-6xl'
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-[#090a0d]">
      <header className="shrink-0 border-b border-white/[0.06] px-3 py-3 sm:px-4 bg-[#08090c]/80 backdrop-blur-sm">
        ...
        <div
          className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-1 p-1 rounded-xl bg-slate-900/60 border border-white/[0.06] w-full max-w-3xl"
          role="tablist"
      ...
      <div
        className={`flex-1 min-h-0 overflow-y-auto px-4 sm:px-6 py-6 w-full mx-auto ${contentMax}`}
      >
        {tab === 'connection' && (
          <>
            ...
            <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900/30 p-4 sm:p-5">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                Web research
              </h3>
              <p className="text-sm text-slate-400 mt-1.5 leading-relaxed">
                Council <span className="text-slate-300">run research</span> uses this backend.
